...
ref=1:dim(a)[1]
for (i in 1:dim(a)[1]){
	sel=which(a[i,10:(dim(a)[2]-10)]=="T" & a[i,11:(dim(a)[2]-9)]=="C" & a[i,12:(dim(a)[2]-8)]=="T" & a[i,13:(dim(a)[2]-7)]=="G" & a[i,14:(dim(a)[2]-6)]=="C" & a[i,15:(dim(a)[2]-5)]=="T" & a[i,16:(dim(a)[2]-4)]=="G" & a[i,17:(dim(a)[2]-3)]=="G" & a[i,18:(dim(a)[2]-2)]=="G")+9
	if(length(sel)==1){
		ref[i]=sel
	} else {
		ref[i]=0
...
for (i in 1:dim(a)[1]){
	if(dim(a)[2]>=ref[i]+90){
		if(ref[i]!=0 & ref[i]-18>0 & ref[i]+90<length(a[i,]) & is.na(a[i,ref[i]+90])==F){
			# 274
			if(a[i,ref[i]-16]=="A"){
				res[i]="I"
			} else if(a[i,ref[i]-16]=="G"){
				res[i]="V"
			} else {
				res[i]="X"
			}
			# 275
			if(a[i,ref[i]-13]=="T" & a[i,ref[i]-11]=="T"){
				res[i]=paste(res[i],"F",sep="")
			} else if(a[i,ref[i]-13]=="C" & a[i,ref[i]-11]=="G"){
				res[i]=paste(res[i],"L",sep="")
			} else {
			...
			# 285
			if(a[i,ref[i]+17]=="A"){
				res[i]=paste(res[i],"T",sep="")
			} else if(a[i,ref[i]+17]=="G"){
			...
			if(a[i,ref[i]+56]=="G"){
				res[i]=paste(res[i],"A",sep="")
			} else if(a[i,ref[i]+56]=="C"){
				res[i]=paste(res[i],"P",sep="")
			} else {
				res[i]=paste(res[i],"X",sep="")
			}
			# 309
